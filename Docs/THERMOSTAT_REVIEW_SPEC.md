# GazeboStove Thermostat System - Comprehensive Review & Correction Specification
...
This document provides a comprehensive review of the GazeboStove thermostat control system, comparing current implementation against expected functionality, identifying all deviations, and providing detailed sequential correction steps. The analysis reveals **critical priority hierarchy issues**, **missing features**, and **incorrect control logic** that prevent proper operation.

**Critical Findings:**
1. **Priority Hierarchy Inversion** - Force Off must be highest priority, currently third
2. **Missing Schedule Feature** - Home Mode Time / Away Mode Time automation not implemented
3. **Incorrect Hysteresis Logic** - ESPHome thermostat platform uses symmetric deadband, user expects asymmetric
4. **Missing Force Off Override** - Emergency Heat doesn't check Force Off state before activating
5. **Thermostat Platform Limitations** - ESPHome thermostat platform doesn't support custom hysteresis calculation

**Impact**: System does not meet safety requirements (Force Off can be overridden) and lacks critical energy-saving automation. Thermostat behavior doesn't match user expectations.
...
3. [Gap Analysis - Deviations from Expected Behavior](#3-gap-analysis---deviations-from-expected-behavior)
4. [Strengths Assessment](#4-strengths-assessment)
5. [Weaknesses Assessment](#5-weaknesses-assessment)
6. [Recommendations](#6-recommendations)
7. [Sequential Correction Steps](#7-sequential-correction-steps)
8. [Detailed Component Specifications](#8-detailed-component-specifications)
...
- **Temperature Sensor**: DS18B20 on GPIO4 (OneWire bus)
- **Display**: Nextion 3.5" on GPIO16/17 (UART)
- **Status LED**: GPIO2

**Configuration Structure:**
- Modular YAML includes (core, hardware, sensors, entities, automation, display, buttons)
- Clean separation of concerns
- Well-documented code

### 1.2 Current Control Logic Implementation

**Location**: `includes/automation.yaml` (lines 28-115)
...
Priority 4 (LOWEST):   Thermostat Algorithm (ESPHome climate platform)
```
...
- **Issue**: This violates user requirement - Force Off should have highest priority
...
- **Location**: `includes/hardware.yaml` (lines 54-76)
- **Issue**: Should be Priority 2, but should respect Force Off

#### Priority 3: Manual Stop (Force Off)
- **Action**: Forces relay OFF indefinitely
- **Switch Type**: Toggle (correctly implemented)
- **Location**: `includes/hardware.yaml` (lines 78-98)
- **Issue**: Should be Priority 1 (HIGHEST), not Priority 3

#### Priority 4: Thermostat Algorithm
- **Platform**: ESPHome `climate` component with `thermostat` platform
- **Location**: `includes/entities.yaml` (lines 90-102)
- **Configuration**:
  ```yaml
  climate:
    - platform: thermostat
  ...
  ```
- **Issue**: ESPHome thermostat platform uses internal symmetric deadband logic, not user's asymmetric hysteresis requirement

### 1.3 User Interface Entities

**Implemented in ESPHome:**
...
| `number.heating_hysteresis` | Number | Deadband (1-5°C) | ⚠️ Defined but not used by thermostat |
...
| `climate.propane_stove_thermostat` | Climate | Thermostat control | ✅ Implemented |
...
**ESPHome Thermostat Platform Behavior:**

The ESPHome `thermostat` platform uses its own internal control logic:
- Symmetric deadband around setpoint
- Built-in hysteresis calculation (not user-configurable)
- Automatic state management
- Does NOT use `number.heating_hysteresis` entity

**Current Temperature Control:**
- ESPHome thermostat platform calculates thresholds internally
- User's `heating_hysteresis` number entity is defined but ignored
- No way to configure asymmetric hysteresis (±H instead of ±H/2)

**Problem**: User expects custom hysteresis logic, but ESPHome thermostat platform doesn't support it.
...
**Note**: This describes ASYMMETRIC hysteresis, not symmetric
...
## 3. Gap Analysis - Deviations from Expected Behavior
...
**Root Cause**: Priority order in `manage_heating` script checks Away Mode first (line 42) before Manual Stop (line 51)

**Location**: `includes/automation.yaml:41-57`
...
**Status**: Documented in `PHASE_4_SIMPLE.md` but not implemented
...
**Problem**: ESPHome thermostat platform uses symmetric deadband, user expects asymmetric hysteresis

**Current Implementation** (ESPHome thermostat platform):
- ESPHome thermostat platform uses internal symmetric deadband
- User's `heating_hysteresis` number entity is defined but NOT USED
- No way to configure custom hysteresis calculation
- Platform calculates thresholds internally (not user-configurable)
...
| Setpoint | Hysteresis | ESPHome Platform | Expected Logic |
|----------|------------|------------------|----------------|
| 22°C | 2°C | ON: <21°C, OFF: >23°C (symmetric) | ON: <20°C, OFF: >24°C (asymmetric) |
| 20°C | 1°C | ON: <19.5°C, OFF: >20.5°C | ON: <19°C, OFF: >21°C |
| 18°C | 3°C | ON: <16.5°C, OFF: >19.5°C | ON: <15°C, OFF: >21°C |

**Impact**:
- Current: ESPHome platform uses its own logic (symmetric, narrower deadband)
- Expected: Custom asymmetric hysteresis (wider deadband), better for propane stove longevity
- User expectation mismatch

**Root Cause**: ESPHome thermostat platform doesn't support custom hysteresis calculation. Must use custom script instead.

**Location**: `includes/entities.yaml:90-102` (climate platform)
...
User presses Emergency Heat → Relay turns ON (Force Off is ignored)
```
...
**Location**: `includes/hardware.yaml:61-69` (manual_run turn_on_action)

### 3.5 Issue #5: Thermostat Platform Limitations

**Problem**: ESPHome thermostat platform doesn't support custom hysteresis calculation

**Current Approach**: Using ESPHome `climate` platform with `thermostat` platform
- Platform handles all control logic internally
- Cannot customize hysteresis calculation
- User's `heating_hysteresis` entity is ignored

**Required Approach**: Custom script-based thermostat control
- Replace climate platform with custom script
- Use `number.heating_hysteresis` entity for asymmetric hysteresis
- Implement custom control logic matching user requirements

**Impact**: HIGH (requires architectural change)

**Location**: `includes/entities.yaml:90-102`
...
- ESPHome thermostat platform uses symmetric deadband
- User's `heating_hysteresis` entity is defined but ignored
- Platform doesn't support custom asymmetric hysteresis
- Requires architectural change (replace climate platform with custom script)
...
⚠️ **Thermostat Platform Mismatch**
- Using ESPHome climate platform doesn't match user requirements
- Platform limitations prevent custom hysteresis
- Should use custom script instead
...
1. **Fix Priority Hierarchy** (CRITICAL - Safety Issue)
   - Reorder priority checks to make Force Off highest priority
   - Update `manage_heating` script
   - Test thoroughly before deployment

2. **Replace Climate Platform with Custom Script** (HIGH - Functional Issue)
   - Remove ESPHome thermostat platform
   - Implement custom thermostat script using `heating_hysteresis` entity
   - Implement asymmetric hysteresis logic (±H instead of ±H/2)
   - Maintain thermostat card functionality in Home Assistant

3. **Add Force Off Check to Emergency Heat** (MEDIUM - Safety Issue)
   - Prevent Emergency Heat activation when Force Off is active
   - Add conditional check in `manual_run` turn_on_action

### 6.2 Short-Term Actions (High Priority)

4. **Implement Schedule Feature** (CRITICAL - Missing Feature)
   - Create Home Assistant time input entities
   - Create Home Assistant automations for time-based triggers
   - Add dashboard card for schedule control

5. **Add Priority Status Indicator** (RECOMMENDED - UX Improvement)
   - Create text sensor showing active priority level
   - Update dashboard to display current control mode
   - Help users understand which mode is active

### 6.3 Long-Term Actions (Enhancements)

6. **Make Emergency Heat Timeout Configurable** (OPTIONAL)
   - Convert 15-minute timeout to Home Assistant number entity
   - Allow user to adjust timeout duration

7. **Add Notification System** (OPTIONAL)
   - Notify user when schedule automations run
   - Alert on priority mode changes
   - Status updates for manual interventions

8. **Add Historical Tracking** (OPTIONAL)
   - Track heating cycles per day
   - Log temperature min/max/average
   - Runtime statistics for optimization
...
**Rationale**:
- ESPHome thermostat platform doesn't support custom hysteresis
- User requires asymmetric hysteresis (±H, not ±H/2)
- Custom script provides full control over logic

**Approach**:
1. Remove `climate` platform from `includes/entities.yaml`
2. Create custom thermostat script in `includes/automation.yaml`
3. Use `number.heating_hysteresis` entity for threshold calculation
4. Implement asymmetric hysteresis logic
5. Create Home Assistant template climate entity to maintain thermostat card UI

**Benefits**:
- Full control over hysteresis calculation
- Matches user requirements exactly
- Can add future enhancements easily

**Drawbacks**:
- More complex implementation
- Must maintain Home Assistant template climate entity for UI

---

## 7. Sequential Correction Steps
...
**Location**: `includes/automation.yaml:28-115`
...
**Specific Changes**:
- Move Manual Stop check to immediately after script start
...
**Location**: `includes/hardware.yaml:61-69`
...
### Step 3: Replace Climate Platform with Custom Thermostat Script (HIGH PRIORITY)

**Objective**: Implement custom asymmetric hysteresis logic matching user requirements
...
3.2. **Create Custom Thermostat Script**

**Location**: `includes/automation.yaml` (add after `manage_heating` script)
...
script:
  - id: thermostat_control
    then:
      - lambda: |-
          // Only run if no higher priority is active
          if (id(manual_stop_active) || 
              id(manual_run_active) || 
              (id(presence_mode).state == "away")) {
            ESP_LOGD("THERMOSTAT", "Skipped - higher priority active");
            return;
          }
          ...
          // Asymmetric hysteresis calculation
          float lower_threshold = desired - hysteresis;
          float upper_threshold = desired + hysteresis;
          
          ESP_LOGD("THERMOSTAT", "Current: %.1f°C, Desired: %.1f°C, "
                   "Hysteresis: %.1f°C, Lower: %.1f°C, Upper: %.1f°C",
                   current_temp, desired, hysteresis,
                   lower_threshold, upper_threshold);
          
          if (current_temp < lower_threshold) {
            // Temperature below threshold - start heating
            if (!id(relay).state) {
              id(relay).turn_on();
              ESP_LOGI("THERMOSTAT", "Heating ON: %.1f°C < %.1f°C",
                       current_temp, lower_threshold);
            }
          } else if (current_temp > upper_threshold) {
            // Temperature above threshold - stop heating
            if (id(relay).state) {
              id(relay).turn_off();
              ESP_LOGI("THERMOSTAT", "Heating OFF: %.1f°C > %.1f°C",
...
3.3. **Add Interval Automation to Run Thermostat**

**Location**: `includes/automation.yaml` (add to interval section)

**Add**:
```yaml
interval:
  # Run thermostat control every 10 seconds
  - interval: 10s
    then:
      - script.execute: thermostat_control
```

3.4. **Create Home Assistant Template Climate Entity**

**File**: Home Assistant `configuration.yaml` (or via UI)
...
**Validation Criteria**:
- ✅ Thermostat script uses `heating_hysteresis` entity
- ✅ Asymmetric hysteresis calculation (desired ± H, not ±H/2)
- ✅ Relay turns ON at (desired - H)
- ✅ Relay turns OFF at (desired + H)
- ✅ Home Assistant thermostat card still works
- ✅ Temperature setpoint changes update `desired_temp` entity

---

### Step 4: Implement Home/Away Schedule (Phase 4)
...
4.1. **Create Time Input Entities in Home Assistant**
...
4.2. **Create Home Mode Automation**
...
4.3. **Create Away Mode Automation**
...
4.4. **Create Dashboard Schedule Card**
...
**Location**: `includes/entities.yaml` (add to text_sensor section)
...
3. Set desired temp = 22°C, hysteresis = 2°C
4. Heat room to 25°C
5. Let temp drop slowly
6. Record temperature when relay turns ON (should be ~20°C, not 21°C)
7. Let heating run until relay turns OFF
8. Record temperature when relay turns OFF (should be ~24°C, not 23°C)

**Expected Result**: Thermostat follows asymmetric hysteresis logic correctly

#### 6.2 Schedule Automation Tests
...
#### 6.4 Force Off Emergency Heat Interaction Test
...
### 8.1 Priority Control Flow Diagram
...
### 8.4 Hysteresis Comparison

**Symmetric (ESPHome Platform - INCORRECT)**:
```
Temperature (°C)
      ↑
  25  │
...
|----------------|--------------|---------------|----------------|-----------------|
| Symmetric (ESPHome) | 21°C | 23°C | 2°C | Higher (more cycles) |
...
- [ ] Thermostat card displays correctly in Home Assistant
...
This specification document identifies **five critical issues** in the GazeboStove thermostat control system:
...
3. **Incorrect Hysteresis Logic** (HIGH): ESPHome thermostat platform doesn't support custom asymmetric hysteresis
4. **Missing Force Off Override** (MEDIUM): Emergency Heat doesn't check Force Off state
5. **Thermostat Platform Limitations** (HIGH): Must replace climate platform with custom script
...
**Estimated Total Effort**: 8-10 hours implementation + 48-72 hours monitoring
