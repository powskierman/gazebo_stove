...
class Component:
        ...
        for attName, attData in self.rawData["att"].items():
            ...
            if attName in Component.attributes.keys():
                ...
                if len(attData) > 4 or includeUnknown == 2:#raw
                    attData = attData.decode("cp1252", errors="ignore")
    ...
    def loadRawProperties(self):
        ...
        while index < len(properties):
            ...
            length = int(length)
            index += 1
            self.rawData[name.decode("cp1252", errors="ignore")] = properties[index : index + length]
            index += length
        for k, v in self.rawData.items():
            if k == "att":
                ...
                for att in v:
                    ...
                    attName = att[:16].rstrip(b"\x00").decode("cp1252", errors="ignore")
                    attData = att[16:]
                ...
                # code lines
                eventCode = (b"\n".join(v)).decode("cp1252", errors="ignore")
...
class PageHeader(Header):
    ...
    def _processData(self, data):
        ...
        self.name         = data[9].decode("cp1252", errors="ignore").rstrip("\x00")
...
class HMIContentHeader(Header):
    ...
    def _processData(self, data):
        ...
        self.size    : int
        self.deleted : bool
        self.name    = data[0].decode("cp1252", errors="ignore").rstrip("\x00")
        self.start   = data[1]
        self.size    = data[2]
        self.deleted = data[3]
    ...
    def __init__(self, HMIFilePath):
        ...
        for obj in self.header.content:
            if obj.isPage():
                ...
                self.programS = self.raw[obj.start : obj.start + obj.size].decode("cp1252", errors="ignore")
            """
            end = obj.start + len(obj)
            s = self.raw[obj.start:end].decode("cp1252", errors="ignore")
            comp = component(s)
